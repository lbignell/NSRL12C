...
G4bool SensitiveDetector::ProcessHits(G4Step* theStep, G4TouchableHistory*){
  ...
  if(theStep->GetTrack()->GetParticleDefinition()->GetParticleName()
    ...
    //UVT Acrylic
      ...
      //G4cout << "Optical Photon Entering UVT Acrylic, logging..." << G4endl;

      if(theStep->GetPostStepPoint()->GetPosition().z()<0){
	//Upstream window
	ProcVec.push_back(windowU);
	//G4cout << "Hit Upstream window!" << G4endl;
      }
      else{
	//Downstream window
...
void SensitiveDetector::EndOfEvent(G4HCofThisEvent*){
  ...
  //accessible to my sensitive detectors and run action? (It would beat this).
...
////////////////////////////////////////////////////////////////////////////////
//The method below is no longer called. The quantum yield is now implemented in
//the MyOpWLS class.
...
G4bool SensitiveDetector::SampleQY(G4double PhotEn){
  ...
  while(itWL != QYdata.at(0).rend()){//Loop shouldn't break with this condition
    ...
    if((prevWL<PhotEn)&&(PhotEn<thisWL)){
      ...
      //if(trial<=theQY){
      //Uniformly reduce/increase the QY by a factor!
      if(trial<=theQY*2){
	return true;
      }
      else{
